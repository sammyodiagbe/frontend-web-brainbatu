...
export const attachPaymentMethod = async ({stripe_customer_id, token_id}: { stripe_customer_id: string, token_id: string}) => {
  try {
    const check = await axios.post("http://localhost:8000/stripe/payments/attach", {
      token_id: token_id,
      stripe_customer_id
  })
